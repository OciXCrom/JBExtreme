...
v1.9-Cr2 (Edit by OciXCrom)
	* Added Simon forwards

v1.9-Cr (Edit by OciXCrom)
	* Added natives
...
public plugin_init()
{
	...
	g_fwdSimonSelected = CreateMultiForward("jb_simon_selected", ET_IGNORE, FP_CELL) 
	g_fwdSimonReset = CreateMultiForward("jb_simon_reset", ET_IGNORE, FP_CELL) 
...
public client_disconnect(id)
{
	if(g_Simon == id)
	{
		...
		ExecuteForward(g_fwdSimonReset, g_iSimonReturn, JBSimonReset_Disconnect)
	}
...
public round_end()
{
	...
	ExecuteForward(g_fwdSimonReset, g_iSimonReturn, JBSimonReset_RoundEnd)
...
public cmd_simon(id)
{
	...
	if(g_SimonAllowed && !g_Freeday && is_user_alive(id) && team == CS_TEAM_CT && !g_Simon)
	{
		...
		ExecuteForward(g_fwdSimonSelected, g_iSimonReturn, id)
		g_PlayerSimon[id]--
...
public cmd_nomic(id)
{
	...
	if(team == CS_TEAM_CT)
	{
		...
		if(g_Simon == id)
		{
			...
			ExecuteForward(g_fwdSimonReset, g_iSimonReturn, JBSimonReset_Nomic)
		}
		if(!is_user_admin(id))
	...
	switch(data[0])
	{
		...
		case('2'):
		{
			if((id == g_Simon) || is_user_admin(id))
			{
				g_Simon = 0
				get_user_name(id, dst, charsmax(dst))
				ExecuteForward(g_fwdSimonReset, g_iSimonReturn, JBSimonReset_Freeday)
...
{
	register_library("jbextreme-crx")
	register_native("get_jail_day", "_get_jail_day")
	register_native("get_players_in_team", "_get_players_in_team")
	register_native("is_block_weapons_enabled", "_is_block_weapons_enabled")
	register_native("is_box_started", "_is_box_started")
	register_native("is_freeday_next", "_is_freeday_next")
	register_native("is_global_freeday", "_is_global_freeday")
	register_native("is_last_denied", "_is_last_denied")
	register_native("is_player_last", "_is_player_last")
	register_native("is_simon_allowed", "_is_simon_allowed")
	register_native("is_user_freeday", "_is_user_freeday")
	register_native("is_user_nomic", "_is_user_nomic")
	register_native("is_user_revolt", "_is_user_revolt")
	register_native("is_user_simon", "_is_user_simon")
	register_native("is_user_simon_talking", "_is_user_simon_talking")
	register_native("is_user_simon_voice", "_is_user_simon_voice")
	register_native("is_user_wanted", "_is_user_wanted")
	register_native("open_jail_doors", "_open_jail_doors")
	register_native("set_user_freeday", "_set_user_freeday")
}

public bool:_is_user_simon(iPlugin, iParams)
	return g_Simon == get_param(1)
	
public bool:_is_user_wanted(iPlugin, iParams)
	return bool:get_bit(g_PlayerWanted, get_param(1))
	
public bool:_is_user_revolt(iPlugin, iParams)
	return bool:get_bit(g_PlayerRevolt, get_param(1))
	
public bool:_is_user_nomic(iPlugin, iParams)
	return bool:get_bit(g_PlayerNomic, get_param(1))
	
public bool:_is_user_freeday(iPlugin, iParams)
	return bool:get_bit(g_PlayerFreeday, get_param(1))
	
public bool:_is_global_freeday(iPlugin, iParams)
	return bool:g_Freeday
	
public bool:_is_freeday_next(iPlugin, iParams)
	return bool:g_FreedayNext
	
public bool:_is_player_last(iPlugin, iParams)
	return g_PlayerLast == get_param(1)
	
public bool:_is_simon_allowed(iPlugin, iParams)
	return bool:g_SimonAllowed
	
public bool:_is_box_started(iPlugin, iParams)
	return bool:g_BoxStarted
	
public bool:_is_user_simon_talking(iPlugin, iParams)
	return bool:get_bit(g_SimonTalking, get_param(1))

public bool:_is_user_simon_voice(iPlugin, iParams)
	return bool:get_bit(g_SimonVoice, get_param(1))

public bool:_is_last_denied(iPlugin, iParams)
	return bool:g_LastDenied
	
public bool:_is_block_weapons_enabled(iPlugin, iParams)
	return bool:g_BlockWeapons
	
public _open_jail_doors(iPlugin, iParams)
	jail_open()
	
public _get_jail_day(iPlugin, iParams)
	return g_JailDay
	
public _get_players_in_team(iPlugin, iParams)
	return get_param(2) ? (g_TeamAlive[CsTeams:get_param(1)]) : (g_TeamCount[CsTeams:get_param(1)])
	
public _set_user_freeday(iPlugin, iParams)
	freeday_set(get_param(2), get_param(1))